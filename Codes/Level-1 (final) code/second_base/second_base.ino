...
void loop() 
{
  ...
  {
    LoRa.write(HC12.read());                  //If data is available, then the data is written in sx-1278.
    while(LoRa.available())                   //Checks if data is available in sx-1278. 
    {
      char in = (char)LoRa.read();            //Convert the data into char datatype. 
      if (in == 'c')
        {
          Serial.print("Error");
        }
